...
import { GiReceiveMoney, GiWallet, GiPayMoney } from 'react-icons/gi';
import { FaUsers } from 'react-icons/fa6';
import { RiPresentationFill, RiBankLine } from 'react-icons/ri';
import { MdOutlineSupportAgent, MdVerifiedUser } from 'react-icons/md';
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
	...
	return (
		<aside
			ref={sidebar}
			className={`absolute left-0 top-0 z-[9999] text-gray-100 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
				sidebarOpen ? 'translate-x-0' : '-translate-x-full'
			}`}
		>
			{/* <!-- SIDEBAR HEADER --> */}
			<div className='flex items-center mt-10 justify-between gap-2 px-6 py-5.5 lg:py-6.5 '>
				<Link href='/' className='cursor-pointer mx-auto'>
					<h3 className='text-2xl font-bold text-white border-dashed sm:text-base md:text-xl lg:flex md:flex sm:flex brand__style w-50'>
						Pyc trade
					</h3>
				</Link>
				...
				<nav className='px-4 py-4 mt-5 lg:mt-9 lg:px-6'>
					...
					<div>
						...
						<ul className='mb-6 flex flex-col gap-1.5'>
													...
													<li>
														<Link
															...
															className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
																pathname === '/users/all-users' && 'text-white'
															}`}
														>
															All Users
														</Link>
							...
							<SidebarLinkGroup
								activeCondition={
									pathname === '/withdraw' || pathname.includes('withdraw')
								}
							>
								{(handleClick, open) => {
									return (
										<React.Fragment>
											...
											>
												<ul className='mt-4 mb-5.5 flex flex-col gap-2.5 pl-6'>
													...
													<li>
														<Link
															href='/withdraw/pending-withdraw'
															className={`group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
										...
										<React.Fragment>
											...
											>
												<ul className='mt-4 mb-5.5 flex flex-col gap-2.5 pl-6'>
													<li>
														<Link
															href='/deposits/all-deposits'
															className={`first-letter:group relative flex items-center gap-2.5 rounded-md px-4 font-medium text-bodydark2 duration-300 ease-in-out hover:text-white ${
																pathname === '/deposits/all-deposits' &&
																'text-green-500'
															}`}
														>
															Deposits
